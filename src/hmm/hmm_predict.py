...
def hmm_predict(filename='hmm.pk1',
    ...
    A,means,covmats,num_samples = utils.pickleLoad(os.path.join(models_dir,filename))
    ...
    def predictz(cqt_list,melody_list,set_name):
        ...
        utils.pickleIt(allmelodies,os.path.join(output_dir,set_name+"_melodies.pk1"))
        utils.pickleIt(allz,os.path.join(output_dir,set_name+"_z.pk1"))
    ...
    utils.pickleIt((train_accuracies,train_voice_det_conf,validation_accuracies,validation_voice_det_conf),os.path.join(models_dir,'accuracies.pk1'))
