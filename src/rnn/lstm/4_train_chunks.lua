...
function train()
   ...
   local sampleshuffle = torch.randperm(numsamples)
   ...
   local truncnumsamples = math.floor(numsamples/batchSize)*batchSize
   for t = 1,truncnumsamples,batchSize do
      -- disp progress
      collectgarbage()
      --xlua.progress(i, numchunks)
      xlua.progress(t, truncnumsamples)
      ...
      local targets = {}
      local targets_c = {}
      for s = t,t+batchSize-1 do
         -- load new sample
         i = lookup[sampleshuffle[s]]
         local input, target, target_c = grabsample(i,TRAIN)
         table.insert(inputs, reshapeInputs(input,opt))
         table.insert(targets, target)
         table.insert(targets_c, target_c)
      end
      ...
      if opt.chroma then
        targets_c = nn.JoinTable(1):forward(targets_c)
        targets_c = targets_c:cuda()
      end
   ...
   -- time taken
   time = sys.clock() - time
   print("\n==> time to learn 1 sample = " .. (time/60) .. ' minutes')
