...
function backwardpass(inputs,targets)
  ...
  local derr_t = transfer_data(torch.ones(1))
  local s_next = nil 
  local tmp_t = model.rnnt[1]:backward({x_t, y_t, s_t},{derr_t, model.ds, dummy})[3]
  g_replace_table(model.ds, tmp_t)
  for i = opt.kL, 1, -1 do
    local x = inputs[{{},i}]
    local y = targets[{{},i}]
    local s = model.s[i - 1]
