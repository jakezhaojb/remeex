...
function lstm(i, prev_c, prev_h, peepholes, num_nodes)
   local function new_input_sum()
      local i2h            = nn.Linear(num_nodes, num_nodes)
      local h2h            = nn.Linear(num_nodes, num_nodes)
   ...
   local function new_input_sum_peepholes()
      ...
      local i2h            = nn.Linear(num_nodes, num_nodes)
      local h2h            = nn.Linear(num_nodes, num_nodes)
      local c2h            = nn.Linear(num_nodes, num_nodes)
      return nn.CAddTable()({i2h(i), h2h(prev_h), c2h(prev_c)})
   end
   if peepholes ~= true then
...
function create_network()
  ...
  for layer_idx = 1, opt.layers do
    ...
    local next_c, next_h = lstm(dropped, prev_c, prev_h, opt.peepholes, opt.rnn_size)
...
if opt.pretrained ~= nil then
  local pretrained_path = paths.concat(opt.save,opt.pretrained,'model_best.net')
  print('>> Loading pre-trained model:',pretrained_path)
  local pretrained = torch.load(pretrained_path)
  local w,dw = pretrained.core_network:getParameters()
  parameters:copy(w)
  print('>> Finished copying pre-trained model parameters, norm check:',parameters:norm())
end
collectgarbage()