...
function doall()
	...
	if not pcall(function()	model = torch.load(paths.concat(results_folder,'model_best.net')) end) then
	...
	local validationFile = opt.validationFile or '5_validate.lua'
	...
	for key,val in pairs(opt.trainParams) do
		...
		dofile (paths.concat(opt.path, validationFile))
