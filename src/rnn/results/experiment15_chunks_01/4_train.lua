...
function getTrainDataSlice(t,cqt,melody,chroma)
...
function train()
   ...
   for s = 1,numsongs do
      ...
      local cqt, melody, chroma = getTrainData(songshuffle[s])
      reset_s()
      segment_length = math.floor(cqt:size(2)/opt.kL)
      for t = 1,segment_length do
        local inputs,targets,targets_c = getTrainDataSlice(t,cqt,melody,chroma)
