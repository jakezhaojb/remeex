...
package.path = package.path .. ';/home/jmj418/torch_utilities/?.lua'
require 'sopt'
...
function getTrainDataSlice(t,cqt,melody)
...
function train()
   ...
   for s = 1,TRAIN.songcount do
      ...
      local cqt, melody = getTrainData(songshuffle[s])
      reset_s()
      segment_length = math.floor(cqt:size(2)/opt.kL)
      for t = 1,segment_length do
        local inputs, targets = getTrainDataSlice(t,cqt,melody)
